...
** Theme Changes
 #+BEGIN_SRC emacs-lisp
   ...
   (defun matiash-toggle-dark-mode ()
     "Toggles TANGO-DARK custom theme. Assumes theme is
   installed"
     (interactive)
     (if (custom-theme-enabled-p 'tango-dark)
	 (progn
...
(setq org-stuck-projects '("+PROJECT/-DONE-FUTURE-CANCELLED" ("NEXT" "WAITING") nil ""))
...
(setq org-agenda-custom-commands
	 ...
	 ((agenda #1="" ((org-agenda-span 1) ;; (org-agenda-use-time-grid nil)
                         ))
          (todo "GOAL" ((org-agenda-overriding-header "\nGoals")))
          (stuck #1="" ((org-agenda-overriding-header "\nStuck Projects") (org-agenda-span 1) (org-agenda-use-time-grid nil)))
        ...
        ("w" "Work Agenda with Current-day and Unscheduled TODOs"
	 ((agenda #1="" ((org-agenda-span 1) (org-agenda-files `(,work-inbox-path)) (org-agenda-use-time-grid nil)
                         ))
          (todo "GOAL" ((org-agenda-overriding-header "\nGoals") (org-agenda-files `(,work-inbox-path))))
          (stuck #1="" ((org-agenda-overriding-header "\nStuck Projects")
                        (org-agenda-span 1)
                        (org-agenda-use-time-grid nil)
                        (org-agenda-files `(,work-inbox-path))))
          (agenda #1="" ((org-agenda-overriding-header "\nOngoing Projects")
                     (org-agenda-skip-function
		      '(org-agenda-skip-entry-if 'nottodo
						 '("PROJECT"))) 
	...
	("n" "Notes"
	 ((todo "NOTE" ((org-agenda-files `(,work-notes-path))))))
        ("p" "Projects"
         ((agenda #1="" ((org-agenda-overriding-header "\nProjects")
                     (org-agenda-skip-function
		      '(org-agenda-skip-entry-if 'nottodo
...
*** Always Save Org Mode Files
    Usually performing actions on tasks causes the buffers to be out
    of date.  That eats up time when I call some command that tries to
    save all the buffers.
    #+BEGIN_SRC emacs-lisp
(defun matiash-always-save-this-file ()
  "Sets a variable so that this file is always saved"
  (if (not (string-match "\\*.*\\*" (buffer-name (current-buffer))))
  (setq buffer-save-without-query t)))
(add-hook 'org-mode-hook 'matiash-always-save-this-file)
    #+END_SRC
* Searching
** Ag
...
(setq ibuffer-saved-filter-groups
	 ...
	 ("Org" (or (mode . org-mode)
		    (filename . "OrgMode")))         
         ("directories" (or (mode . dired-mode)))
         ("shells" (or (mode . shell-mode) 
                       (mode . eshell-mode)))
	 ("emacs-config" (or (filename . ".emacs.d")
			     (filename . ".emacs")))
         ("Emacs Buffers" (or (starred-name . "\**\*")))
...
(setq display-buffer-alist
      '(("\\*Org Src.*\\*" . (display-buffer-same-window))
        ...
        ("\\*Ibuffer\\*" (display-buffer-in-side-window) (side . left) (window-width . 0.5))
        ("\\*Flycheck errors\\*" (display-buffer-in-side-window) (side . bottom) (slot . -1))
        ("\\*notmuch-.*\\*" (display-buffer-same-window))
        ("\\*Org Agenda.*\\*" (display-buffer-at-bottom) (window-height . 0.25))
        ("\\*scratch\\*" (display-buffer-same-window))
        ("\\*grep\\*" (display-buffer-in-side-window) (window-height . 0.20) (slot . 0))
        ("shell-.*" (display-buffer-in-side-window) (side . bottom)  (window-height . 0.20) (slot . 0) (window-parameters . ((no-delete-other-windows . t))))
        ("\\*compilation\\*" (display-buffer-in-side-window) (side . bottom)  (window-height . 0.20) (slot . 0) (window-parameters . ((no-delete-other-windows . t))))
        ("\\*ansi-term.*\\*" (display-buffer-in-side-window) (side . bottom)  (window-height . 0.20) (slot . 0) (window-parameters . ((no-delete-other-windows . t))))
...
** Integrate with Org Mode Better
#+BEGIN_SRC emacs-lisp
(define-key shell-mode-map (kbd "C-c C-x") nil)
(require 'sh-script)
(define-key sh-mode-map (kbd "C-c C-x") nil)
#+END_SRC

** Setup Coterm Mode
Looks like this mode allows for good terminal integration for
shell-mode!
#+begin_src emacs-lisp
(use-package coterm
  :ensure t
...
#+BEGIN_SRC emacs-lisp
(add-hook 'isearch-mode-end-hook (lambda () (if (and isearch-other-end (> (point) isearch-other-end)) (goto-char isearch-other-end))))
#+END_SRC
...
This configuration does help - making it the default:
#+BEGIN_SRC emacs-lisp
(matiash-toggle-line-spacing)
#+END_SRC
...
** Olivetti Mode
I really like this mode, so let's go ahead and map it to a key
#+BEGIN_SRC emacs-lisp
...
** Call Org Capture in Side Window
#+BEGIN_SRC emacs-lisp
(defadvice org-capture (before hide-side-windows)
  (if (window-with-parameter 'window-side nil (selected-frame))
      (window-toggle-side-windows)))
#+END_SRC
