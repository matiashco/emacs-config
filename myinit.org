...
(global-set-key (kbd "<f10>") 'toggle-dark-mode)
 #+END_SRC
* Navigation
** Avy
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
** Using Registers
*** Store to Preset Register
Sets point to preset register and allows for quick jumping to that register,
switching the register to the point switched to. Note that I commented out the
jumping to test my new function that cycles through all possible registers.
#+BEGIN_SRC emacs-lisp
...
(defun cycle-point-across-registers ()
  (interactive)
  "Move point across all possible unique markers in register list."
  (let ((valid-marker-list (seq-uniq
    ...
    (register-val-jump-to (car (or valid-marker-list-copy valid-marker-list)) nil)))
(global-set-key (kbd "C-,") 'cycle-point-across-registers)
#+END_SRC
** Frames
...
(global-set-key (kbd "C-M-u") 'forward-up-list)
#+END_SRC

** Switching Between Buffers
I usually end up working with two files and going back and forth between
them. Adding a keybinding to that easily.
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
* Web Browser (EWW)
Trying to use this more to quickly find webpage.
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
** EWW Variables
*** Set EWW as default web browser
I like this because it gives me a clean look for webpages. That being said, I
should find a way to use firefox for certain pages too.
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
* Elfeed
This package is used to read the news. I tried it out for a bit and love how it
...
(use-package elfeed
  ...
  (setq elfeed-feeds
	'(
	  ...
	  "https://www.boston.com/tag/local-news/?feed=rss")))
#+END_SRC
* TODOS
Try out the following keybindings:
#+BEGIN_SRC emacs-lisp
