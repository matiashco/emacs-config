...
(setq display-buffer-alist
      '(("\\*Org Src.*\\*" . (display-buffer-same-window))
        ...
        ("\\*info\\*" (display-buffer-in-child-frame))
        ("\\*vc-log\\*" (display-buffer-same-window))
        ("\\*.*diff\\*" (display-buffer-in-side-window) (side . right) (window-width . 0.5))
...
(defun matiash-toggle-eshell-mode ()
  ...
  (if (string= major-mode "eshell-mode")
    ...
    (eshell)))
(global-set-key (kbd "s-s") 'matiash-toggle-eshell-mode)
#+END_SRC
* PDF View
Used to view PDF's
#+BEGIN_SRC emacs-lisp
